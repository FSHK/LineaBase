      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-9 ftco-animate text-center">
            ...
            <p class="breadcrumbs"><span class="mr-2"><a href="/">Home <i class="ion-ios-arrow-forward"></i></a></span><span class="mr-2"><a href="{{route('lines')}}">Administradores <i class="ion-ios-arrow-forward"></i></a></span><span> Analisis de Datos</span></p>
          </div>
        </div>
      </div>
    </section>
	<section class="ftco-section ftco-no-pt ftc-no-pb">
		<div class="row justify-content-center mb-5 pb-2">
          <div class="col-md-8 text-center heading-section ftco-animate">
          	<br/>
            <h2 class="mb-4"><span>Encuesta</span> {{$Linea->Nombre}}</h2>
            <p>{{App\Transacciones::Where('transacciones.linea_id','=',$Linea->id)->count()}} Entrevistados</p>
          </div>
        </div>
		<div class="container">
			<div class="row">
				<div class="col-md-10 wrap-about py-10 pr-md-10 ftco-animate">
					<h2 class="mb-4">Promedio de evaluacion de las competencias</h2>
					<div class="column left">
						<table class="table table-borderless table-hover table-responsive">
						  <thead>
						    <tr class="table-primary">
						      <th scope="col" colspan="5" style="text-align: center;">Competencia</th>
						      <th scope="col" style="text-align: center;">Promedio</th>
						    </tr>
						  </thead>
						  <tbody>
						  <p style="display: none;">{{$collection = collect([])}}</p>
						  	@foreach($Competencias as $C)
						  	<tr>
						      <th scope="row" colspan="5">{{$C->Nombre}}</th>
						      <td><a href="{{route('ops',[$C->id])}}" class="btn btn-success">{{round(App\Transacciones::Where('transacciones.competencia_id','=',$C->id)->pluck('valoracion')->avg(), 2)}}</td>
						      <p style="display: none;">{{$collection->push(round(App\Transacciones::Where('transacciones.competencia_id','=',$C->id)->pluck('valoracion')->avg(), 2))}}</p>
						  	@endforeach
						    </tr>
						  	<th scope="row" colspan="5">Promedio Total General</th>
						  	<td><b>{{$collection->avg()}}</b></td>
						  </tbody>
						</table>
						<br/>
					</div>
				...
				<script type="text/javascript">
					Highcharts.chart('container', {
					    ...
					    xAxis: {
					        categories: [@foreach($Competencias as $C)
									     '{{$C->Nombre}}',
									     @endforeach],
					    ...
					    series: [{
					        name: 'Ideal',
					        data: [@foreach($Competencias as $C)
					        		6,
					        	   @endforeach],
					        pointPlacement: 'on'
					    }, {
					        name: 'Promedio',
					        data: [@foreach($Competencias as $C)
					        		{{round(App\Transacciones::Where('transacciones.competencia_id','=',$C->id)->pluck('valoracion')->avg(), 2)}},
					        	   @endforeach],
					        pointPlacement: 'on'
					    },{
					        name: 'Minimo',
